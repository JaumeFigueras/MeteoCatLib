...
from .weather_station import WeatherStationVariableStatusAssociation
from .variable import Variable
from .variable import VariableStatus
import datetime
import dateutil.parser
import sqlalchemy.exc


def get_weather_station(db_session, station_code):
    """
    ...
    :param station_code: Identifying code of the station for the Meteo Cat agency
    :type station_code: str
    :return: The Variable object or None if it was not found in the database
    :rtype: Union[WeatherStation | None]
    """
    try:
        station = db_session.query(WeatherStation)\
...
def get_variables(db_session, station_code=None, active_on_date=datetime.datetime.utcnow()):
    """
    Get a list of all the variables objects that meet the conditions described by the parameters

    :param db_session: Database session that is generated by SQLAlchemy ORM
    :type db_session: sqlalchemy.orm.Session
    :param station_code: Sets if the list include all the weather stations or only that are in an active status at
    the given date
    :type station_code: str
    :param active_on_date: Sets the date when the station has to be in an active status
    :type active_on_date: Union[datetime, str]
    ...
    if station_code is None:
        return db_session.query(Variable)\
            .all()
    else:
        if type(active_on_date) is str:
            active_on_date = dateutil.parser.parse(active_on_date)
        # noinspection PyComparisonWithNone, PyPep8
        return db_session.query(Variable)\
            .join(WeatherStationVariableStatusAssociation)\
            .join(WeatherStation)\
            .filter(WeatherStation.code == station_code) \
            .join(VariableStatus) \
            .filter((VariableStatus.code == 2)
                    & (VariableStatus.from_date <= active_on_date)
                    & ((active_on_date <= VariableStatus.to_date) | (VariableStatus.to_date == None)))\
            .all()  # noqa: E711
